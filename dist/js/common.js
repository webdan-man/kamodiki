...
$(document).ready(function() {
	...
	$('.slick-slider .slick-arrow,.slick-slider button').click(function() {
		$('.slick-slider .video').html(' ').removeClass('no-button');
	})
	...
	$('section.price input').click(function() {
		$('section.price label').removeClass('active');
		$(this).next('label').addClass('active');
	});

	// Background for dots in section.price
	if($(window).width() < 768) {
		for (var i = 10; i < 100; i++) {
			var slideImg =	$(".slick-slider-price .slick-slide[aria-describedby='slick-slide" + i + "']" + ' img').attr('src')
			//	slideImg = $(slide).attr('src');
			$('.slick-slider-price .slick-dots li#slick-slide' + i + ' ' + 'button').css({
	...
	$('.slick-slider-price .slick-slide').click(function() {
		$(this).closest('.slick-slider-price').slick('slickNext');
	});
	...
	$('.slick-slider-comment,.slick-slider-fotos').on('swipe', function(event, slick, direction){
		$('section.fotos span.current').text(slick['currentSlide'] + 1);
		if (direction === 'right') {
			$('.saw').css({transform:'rotate(90deg)',transition: 'all,.8s'});
			setTimeout(function() {
